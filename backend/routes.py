...
def student_obj(s,private=True):
    ...
    x={"id":s.id,"student_id":s.student_id,"name":s.name,"photo":s.photo,"gender":s.gender,"dob":s.dob.isoformat() if s.dob else None,"phone":s.phone,"school_name":s.school_name,"admission_date":s.admission_date.isoformat() if s.admission_date else None,"status":s.status,"classes":cls,"current_month_status":current_month_status}
...
@roles("teacher")
def my_classes():
    t=teacher_for_user(); now=datetime.now(ZoneInfo("Asia/Kolkata")); out=[]
    for a in TeacherClass.query.filter_by(teacher_id=t.id,status="active").all():
        c=Class.query.get(a.class_id)
        if not c or c.status!="active": continue
        item=class_obj(c)
        st=attendance_state(a,now)
        item["attendance_active"]=st["active"]
        item["attendance_schedule"]={"day_of_week":st["day_of_week"],"start_time":st["start_time"],"end_time":st["end_time"]}
        out.append(item)
    return ok(out)
